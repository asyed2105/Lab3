...
public class JSONTranslationExample {
    ...
    public String getCountryNameTranslation(String countryCode, String languageCode) {
        for (int i = 0; i < jsonArray.length(); i++) {
            JSONObject country = jsonArray.getJSONObject(i); // Get each country as a JSONObject from the array
            if (country.getString("alpha3").equals(countryCode)){
                return country.optString(languageCode, "Translation not there");
            }
        }
        return "Country not there";
    }
